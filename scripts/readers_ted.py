...
def getATL07(f,beam):
    ...
    df07=pd.DataFrame({'lats':lats,'lons':lons,'heights':heights,'dt':dt,'seg_x':seg_x,'conf':conf,'stype':stype,'ssh_flag':ssh_flag,     'gauss':gauss,'photon_rate':photon_rate,'cloud':cloud,'mss':mss,'ocean':ocean_tide,'lpe':lpe_tide,'ib':ib})
    return df07
...
def getATL10(f,beam):
    ...
    freeboard_confidence = f[beam+'freeboard_beam_segment/beam_freeboard/beam_fb_confidence'][:]
    freeboard_quality = f[beam+'freeboard_beam_segment/beam_freeboard/beam_fb_quality_flag'][:]
    conf = f[beam+'/sea_ice_segments/heights/height_segment_confidence'][:]
    stype = f[beam+'/sea_ice_segments/heights/height_segment_type'][:]
    ssh_flag = f[beam+'/sea_ice_segments/heights/height_segment_ssh_flag'][:]
    
    df10=pd.DataFrame({'lats':lats,'lons':lons,'freeboard':freeboard,'freeboard_quality':freeboard_quality,
                    'heights':heights,'dt':dt,'seg_x':seg_x,'conf':conf,'stype':stype,'ssh_flag':ssh_flag})
